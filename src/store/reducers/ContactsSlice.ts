...
const contactsSlice = createSlice({
  ...
  reducers: {
    setCurrentContact(state, action) {
      state.currentContact = state.contactsList.filter(
        (item) => item['id'] === action.payload
      )[0]
    },
    setNewCurrentContact(state, action) {
      state.currentContact = {
        ...initialState.currentContact,
        id: action.payload,
      }
    },
    ...
    editContact(state) {
      state.contactsList = state.contactsList.map((item) =>
        item['id'] === state.currentContact.id ? state.currentContact : item
      )
    },
